...
public class _01_GetAllRowColumn extends JDBCParent {
    ...
    public void test2() throws SQLException {
        ...
        ResultSetMetaData rsmd=rs.getMetaData();  //veri harici diğer bilgiler (meta)

        int columnCount=rsmd.getColumnCount();

        rs.next();  //ilk satıra gittim
        for (int i = 1; i < columnCount; i++) {  //kaç tane kolon varsa hepsindeki bilgileri yazdırabilirm
    ...
    @Test
    public void test3_2() throws SQLException {// daha düzenli hali için printf ile yapıyoruz.
        ...
        while (rs.next()){
            for (int i = 1; i <= rsmd.getColumnCount(); i++) {
                System.out.printf("%-20s",rs.getString(i));  //sola dayalı yaz 20 hane kullan
            }
            System.out.println();
        }
    ...
    public void test4(){
        ...
        List<List<String>> donenLİst=getListData(sorgu);
        System.out.println("donenLİst = " + donenLİst);
    }
    ...
    // TODO (Challenge) : DB den Customer tablosunu okuyarak , yeni bir EXCELE yazdırınız.    (YAPMADIM)
